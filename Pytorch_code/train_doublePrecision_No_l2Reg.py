...
def trainModel(path2Data,seed,iterationNum,GPU ,mode = 'train', nb_epoch = 20, batch_size = 16, img_width = 400,img_hight= 400, ch = 1,saveModelOption='last'):
    ...
    if not os.path.exists(os.path.join(expPath,'pytorch_exp_May2021_saveEveryEpochModel_doublePrecision_highLR_0.001_GAIVI')):
        os.makedirs(os.path.join(expPath,'pytorch_exp_May2021_saveEveryEpochModel_doublePrecision_highLR_0.001_GAIVI'))
    taskPath = os.path.join(expPath,'pytorch_exp_May2021_saveEveryEpochModel_doublePrecision_highLR_0.001_GAIVI')
    if os.path.exists(os.path.join(taskPath,task,fold,'Models',fold+'_weights2'+'.pt')) and mode == 'train':
    ...
    if mode == 'train':
        ...
        epoch_loss_dice = []
        #path2Model = os.path.join(taskPath,task,fold,'Models',fold+'_Epoch20_weights'+'.pt')
        ...
        for epoch in range(1,nb_epoch+1):
            ...
            df_max['init_max'] = listOfWeightsMax
            df_max['init_min'] = listOfWeightsMin
            df_max.to_csv(os.path.join(taskPath,task,fold,'Summary','Epoch'+str(epoch)+'_maxPerLayer2.csv'))
            ...
            if epoch_loss < best_loss:
                ...
                try:
                    ...
                    torch.save(best_model.state_dict(), os.path.join(taskPath,task,fold,'Models',fold+'_weights2.pt'))
                except:
        ...
        if not os.path.exists(os.path.join(taskPath,task,fold,'Models',fold+'_weights2'+'.pt')):
        ...
        for phase in ['test']:
            start_time = time.time()
            path2Model = os.path.join(taskPath,task,fold,'Models',fold+'_weights2'+'.pt')
            pred_dir_test = os.path.join(taskPath,task,fold,'PredictedMasks2',fold+'_'+phase+'_predMasks')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-GPU', '--GPU_num', help ='GPU number (Based on nvidia-smi indexing) to use in training/testing model ', required= False)
