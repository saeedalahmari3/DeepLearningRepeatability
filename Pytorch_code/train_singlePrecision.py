...
#   This code has been created by SAEED ALAHMARI - Najran University and University of South Floirda , Computer Science departments .
...
def trainModel(path2Data,seed,iterationNum,GPU ,mode = 'train', nb_epoch = 20, batch_size = 16, img_width = 400,img_hight= 400, ch = 1,saveModelOption='last'):
    ...
    if not os.path.exists(os.path.join(expPath,'pytorch_exp_SinglePrecision')):
        os.makedirs(os.path.join(expPath,'pytorch_exp_SinglePrecision'))
    taskPath = os.path.join(expPath,'pytorch_exp_SinglePrecision')
    ...
    if mode == 'train':
        ...
        optim = torch.optim.Adam(model.parameters(),lr=1e-4,eps=1e-7,amsgrad=False, weight_decay= 1e-3)
        ...
        for epoch in range(1,nb_epoch+1):
            ...
            for phase in ['train']:
                ...
                for X,y in zip(train_loader,trainMasks_loader):
                    X = X.to(device='cuda',dtype=torch.float)  # [N, 1, H, W]
                    y = y.to(device='cuda',dtype=torch.float)  # [N, H, W] with class indices (0, 1)
                    optim.zero_grad()
            ...
            if epoch_loss < best_loss:
                ...
                try:
                    ...
                    torch.save(best_model.state_dict(), os.path.join(taskPath,fold,'Models',fold+'_weights.pt'))
                except:
        ...
        if not os.path.exists(os.path.join(taskPath,fold,'Models',fold+'_weights'+'.pt')):
            print('Model for does not exist')
            sys.exit()
        
        for phase in ['test']:
            start_time = time.time()
            path2Model = os.path.join(taskPath,fold,'Models',fold+'_weights'+'.pt')
            pred_dir_test = os.path.join(taskPath,fold,'PredictedMasks',fold+'_'+phase+'_predMasks')
