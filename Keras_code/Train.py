...
def trainModel(path2Data,seed,iterationNum, GPU, precision ,mode = 'train', nb_epoch = 20, batch_size = 4, img_width = 400,img_hight= 400):
    ...
    if precision == 'float32':
        expPath = os.path.join(expPath,'KerasTF_SinglePrecision')
    elif precision == 'float64':
        expPath = os.path.join(expPath,'KerasTF_DoublePrecision')
    else:
        print('Precision is unknown')
        sys.exit()

    if not os.path.exists(expPath):
        os.makedirs(os.path.join(expPath,task,'Models'))
    if not os.path.exists(os.path.join(expPath,task,'Summary')):
        os.makedirs(os.path.join(expPath,task,'Summary'))
    taskPath = os.path.join(expPath,task)
    ...
    if mode == 'train':	
        ...
        f = open(os.path.join(taskPath,'Summary','training_time.txt'),'w')
        ...
        f = open(os.path.join(taskPath,'Summary','testing_time.txt'),'w')
